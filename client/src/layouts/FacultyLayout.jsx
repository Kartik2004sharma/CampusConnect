...
import { LogOut, LayoutDashboard, FileText, Bell, Menu, X, Users, MessageSquare } from 'lucide-react';
    ...
    <div className="flex h-screen bg-surface text-text overflow-hidden relative">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-surface-2 border-r border-white/10 flex flex-col z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full uppercase tracking-wider">{user?.role}</span>
            </div>
          </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          <NavLink to="/faculty/dashboard" className={navLinkClass} onClick={() => setIsMobileMenuOpen(false)}><LayoutDashboard size={20} /> Dashboard</NavLink>
          <NavLink to="/faculty/mentorship" className={navLinkClass} onClick={() => setIsMobileMenuOpen(false)}><Users size={20} /> Mentorship</NavLink>
          <NavLink to="/faculty/grievances" className={navLinkClass} onClick={() => setIsMobileMenuOpen(false)}><FileText size={20} /> Grievances</NavLink>
          <NavLink to="/faculty/notices" className={navLinkClass} onClick={() => setIsMobileMenuOpen(false)}><MessageSquare size={20} /> Notices</NavLink>
        </nav>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-surface z-30 relative">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-sora text-lg font-bold tracking-tight text-white hidden md:block">Campus<span className="text-primary">Connect</span> <span className="text-gray-400 text-sm font-normal">Faculty</span></h1>
          </div>
